...
function executeButton_Callback(~, ~)
    ...
    calculate(theta, phi, portName, Delay, Rate, Size, N, M,frequency,index);  % Ensure the calculate function uses this portName
    updateStatusText(fig, 'Calculation completed.');
end

function calculate(theta, phi, portName, Delay, Rate, Size, N, M,frequency,index)
    % Calculation logic here (simplified for demonstration)
    disp(['Calculating with theta: ', num2str(theta), ', phi: ', num2str(phi), ', port: ', portName]);
...
%phi=60;
    ...
    % Dimensions based on wavelength
    du = 11.30e-3; % Element spacing
    %DN = 6.6 * lambda; % Size in direction N
    %DM = 4.9 * lambda; % Size in direction M

    % Compute displacement components
    dsx = du * k * (cos(phiR) * sin(thetaR));
    dsy = du * k * (sin(phiR) * sin(thetaR));

    %M = round(DM / du);
    %N = round(DN / du);
%M=10;N=10;
    % Initialize matrix for phase configuration
    B = zeros(M, N);

    % Compute phase matrix
    for i = 1:M
        for j = 1:N
            B(i, j) = mod((i * dsx + j * dsy), 2 * pi);
            [~, p] = min(abs(E - B(i, j))); % Quantize the phase
            B(i, j) = E(p);
        end
    end
